...
class Equalizer:
    def set_equalizer_mode(self, mode: str) -> None:
        available_modes = ["Rock", "Pop", "Jazz", "Classical", "Normal"]
        if mode in available_modes:
            print(f"Estableciendo modo de ecualizador: {mode}")
        else:
            print(f"Modo de ecualizador no válido: {mode}. Modos disponibles: {', '.join(available_modes)}")

    def reset_equalizer(self) -> None:
        print("Restableciendo ecualizador a configuración predeterminada")
...
class MediaPlayerFacade:
    ...
    def play_media(self, file: str) -> None:
        ...
        if file_type == "audio":
            ...
            # Aplicar ecualizador por defecto para audio
            self.equalizer.set_equalizer_mode("Normal")
    ...
    def stop_media(self) -> None:
        ...
        if self.current_media_type == "audio":
            ...
            self.equalizer.reset_equalizer()
    ...
    # Nuevo método para configurar el modo del ecualizador
    def set_equalizer_mode(self, mode: str) -> None:
        if self.current_media_type == "audio":
            self.equalizer.set_equalizer_mode(mode)
        else:
            print("El ecualizador solo está disponible durante la reproducción de audio")
...
def main():
    ...
    # Reproducir el siguiente elemento de la lista (audio)
    media_player.play_next()

    # Configurar modo de ecualizador
